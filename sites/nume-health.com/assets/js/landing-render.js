/**
 * Nume Health — Landing Page renderer
 *
 * Reads `window.NUME_CONFIG` (set by landing-config.js) and writes values
 * into matching `data-bind="path.to.value"` elements.
 *
 * Supports:
 ...
 *   - show/hide based on truthy values: <div data-bind-show="product.firstMonthPrice">...</div>
 *   - tracking events: <a data-track="cta_click">...</a>
 *
 * Tracking is privacy-safe: it ONLY emits the event NAME, no PHI.
 * If `gtag` or `fbq` are present they will receive the generic event;
 * otherwise it falls back to console.debug.
 */
(function () {
  "use strict";
  var cfg = window.NUME_CONFIG || {};
  ...
  function track(eventName) {
    ...
    if (typeof console !== "undefined" && console.debug) {
      console.debug("[nume.track]", eventName);
    }
  }
  window.numeTrack = track;
